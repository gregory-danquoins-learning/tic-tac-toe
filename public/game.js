// Récupération du nom du joueur dans localStorage.
...
let symbol;      // Symbole du joueur ("X" ou "O")
let yourTurn = false; // Indique si c’est au joueur actuel de jouer

// Affichage du nom du joueur dans la page
document.getElementById("playerName").textContent = playerName;

// Fonction qui crée le plateau de jeu , elle reçoit l’état actuel du board 
function createBoard(board) {
  const container = document.getElementById("board");
  container.innerHTML = ""; // On vide l’ancien board

  // boucle pour parcourir les lignes et colonnes
  for (let row = 0; row < 3; row++) {
    for (let col = 0; col < 3; col++) {
      const cell = document.createElement("div"); // Création d’une case
      cell.className = "cell"; // Style CSS
      ...
      // Si la case est vide, on autorise le clic 
      if (board[row][col] === "") {
...
function play(row, col) {
  // On bloque si ce n’est pas son tour
  if (!yourTurn) return;
